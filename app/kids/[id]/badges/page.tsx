    ...
    <main className="p-5 max-w-sm mx-auto">
      ...
      {earned.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-ink-secondary font-medium">No badges yet — keep going!</p>
        </div>
      ) : (
        <div className="grid grid-cols-3 gap-4">
          {store.badges.filter(b => earnedIds.has(b.id)).map(badge => {
            ...
            return (
              <div key={badge.id} className="bg-white rounded-2xl p-4 text-center shadow-card flex flex-col gap-1">
                <span className="text-4xl">{badge.icon}</span>
                <p className="font-bold text-ink-primary text-xs leading-tight">{badge.name}</p>
                {record && (
